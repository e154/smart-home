// GENERATED BY THE COMMAND ABOVE; DO NOT EDIT
// This file was generated by swaggo/swag at
// 2019-03-16 11:10:05.302567 +0700 +07 m=+0.256936285

package docs

import (
	"bytes"

	"github.com/alecthomas/template"
	"github.com/swaggo/swag"
)

var doc = `{
    "swagger": "2.0",
    "info": {
        "description": "Smart Home System",
        "title": "Smart home API",
        "contact": {
            "name": "Alex Filippov",
            "url": "https://e154.github.io/smart-home/",
            "email": "support@e154.ru"
        },
        "license": {
            "name": "MIT License",
            "url": "https://raw.githubusercontent.com/e154/smart-home/master/LICENSE"
        },
        "version": "1"
    },
    "host": "{{.Host}}",
    "basePath": "/api/v1",
    "paths": {
        "/": {
            "get": {
                "consumes": [
                    "text/plain"
                ],
                "produces": [
                    "text/plain"
                ],
                "tags": [
                    "index"
                ],
                "summary": "index page",
                "responses": {
                    "200": {}
                }
            }
        },
        "/access_list": {
            "get": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "auth"
                ],
                "summary": "Get user access list",
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ResponseAccessList"
                        }
                    },
                    "403": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "500": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    }
                }
            }
        },
        "/device": {
            "post": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "device"
                ],
                "summary": "Add new device",
                "parameters": [
                    {
                        "description": "device params",
                        "name": "device",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.NewDeviceModel"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.NewObjectSuccess"
                        }
                    },
                    "400": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "401": {
                        "description": "Unauthorized"
                    },
                    "500": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    }
                }
            }
        },
        "/device/{id}": {
            "get": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "description": "Get device by id",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "device"
                ],
                "summary": "Show device",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "Device ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.DeviceModel"
                        }
                    },
                    "400": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "401": {
                        "description": "Unauthorized"
                    },
                    "404": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "500": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    }
                }
            },
            "put": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "description": "Update device by id",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "device"
                ],
                "summary": "Update device",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "Device ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "Update device",
                        "name": "device",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.UpdateDevice"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.DeviceModel"
                        }
                    },
                    "400": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "401": {
                        "description": "Unauthorized"
                    },
                    "404": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "500": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    }
                }
            },
            "delete": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "description": "Delete device by id",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "device"
                ],
                "summary": "Delete device",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "Device ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ResponseSuccess"
                        }
                    },
                    "400": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "401": {
                        "description": "Unauthorized"
                    },
                    "404": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "500": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    }
                }
            }
        },
        "/device_action": {
            "post": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "device_action"
                ],
                "summary": "Add new device action",
                "parameters": [
                    {
                        "description": "device action params",
                        "name": "device_action",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.NewDeviceAction"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.DeviceAction"
                        }
                    },
                    "400": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "500": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    }
                }
            }
        },
        "/device_action/{id}": {
            "get": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "description": "Get device action by id",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "device_action"
                ],
                "summary": "Show device action",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "DeviceAction ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.DeviceAction"
                        }
                    },
                    "400": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "404": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "500": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    }
                }
            },
            "put": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "description": "Update device action by id",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "device_action"
                ],
                "summary": "Update device action",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "DeviceAction ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "Update device action",
                        "name": "action",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.UpdateDeviceAction"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ResponseSuccess"
                        }
                    },
                    "400": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "404": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "500": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    }
                }
            },
            "delete": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "description": "Delete device action by id",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "device_action"
                ],
                "summary": "Delete device action",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "DeviceAction ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ResponseSuccess"
                        }
                    },
                    "400": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "404": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "500": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    }
                }
            }
        },
        "/device_action1/search": {
            "get": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "description": "Search device_action by name",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "device_action"
                ],
                "summary": "Search device_action",
                "parameters": [
                    {
                        "type": "string",
                        "description": "query",
                        "name": "query",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "default": 10,
                        "description": "limit",
                        "name": "limit",
                        "in": "query",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "default": 0,
                        "description": "offset",
                        "name": "offset",
                        "in": "query",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.SearchDeviceActionResponse"
                        }
                    },
                    "400": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "404": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "500": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    }
                }
            }
        },
        "/device_actions/{id}": {
            "get": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "description": "Get device list",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "device_action"
                ],
                "summary": "DeviceAction list",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "Device ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "$ref": "#/definitions/models.DeviceAction"
                            }
                        }
                    },
                    "400": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "404": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "500": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    }
                }
            }
        },
        "/device_state": {
            "post": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "device_state"
                ],
                "summary": "Add new device state",
                "parameters": [
                    {
                        "description": "device state params",
                        "name": "device_state",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.NewDeviceState"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.DeviceState"
                        }
                    },
                    "400": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "500": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    }
                }
            }
        },
        "/device_state/{id}": {
            "get": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "description": "Get device state by id",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "device_state"
                ],
                "summary": "Show device state",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "DeviceState ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.DeviceState"
                        }
                    },
                    "400": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "404": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "500": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    }
                }
            },
            "put": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "description": "Update device state by id",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "device_state"
                ],
                "summary": "Update device state",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "DeviceState ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "Update device state",
                        "name": "state",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.UpdateDeviceState"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ResponseSuccess"
                        }
                    },
                    "400": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "404": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "500": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    }
                }
            },
            "delete": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "description": "Delete device state by id",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "device_state"
                ],
                "summary": "Delete device state",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "DeviceState ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ResponseSuccess"
                        }
                    },
                    "400": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "404": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "500": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    }
                }
            }
        },
        "/device_states/{id}": {
            "get": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "description": "Get device list",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "device_state"
                ],
                "summary": "DeviceState list",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "Device ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "$ref": "#/definitions/models.DeviceState"
                            }
                        }
                    },
                    "400": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "404": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "500": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    }
                }
            }
        },
        "/devices": {
            "get": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "description": "Get device list",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "device"
                ],
                "summary": "Device list",
                "parameters": [
                    {
                        "type": "integer",
                        "default": 10,
                        "description": "limit",
                        "name": "limit",
                        "in": "query",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "default": 0,
                        "description": "offset",
                        "name": "offset",
                        "in": "query",
                        "required": true
                    },
                    {
                        "type": "string",
                        "default": "DESC",
                        "description": "order",
                        "name": "order",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "default": "id",
                        "description": "sort_by",
                        "name": "sort_by",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.DeviceListModel"
                        }
                    },
                    "400": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "401": {
                        "description": "Unauthorized"
                    },
                    "404": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "500": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    }
                }
            }
        },
        "/devices/search": {
            "get": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "description": "Search device by name",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "device"
                ],
                "summary": "Search device",
                "parameters": [
                    {
                        "type": "string",
                        "description": "query",
                        "name": "query",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "default": 10,
                        "description": "limit",
                        "name": "limit",
                        "in": "query",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "default": 0,
                        "description": "offset",
                        "name": "offset",
                        "in": "query",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.SearchDeviceResponse"
                        }
                    },
                    "400": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "401": {
                        "description": "Unauthorized"
                    },
                    "404": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "500": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    }
                }
            }
        },
        "/flow": {
            "post": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "flow"
                ],
                "summary": "Add new flow",
                "parameters": [
                    {
                        "description": "flow params",
                        "name": "flow",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.NewFlowModel"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.NewObjectSuccess"
                        }
                    },
                    "400": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "401": {
                        "description": "Unauthorized"
                    },
                    "500": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    }
                }
            }
        },
        "/flow/{id}": {
            "get": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "description": "Get flow by id",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "flow"
                ],
                "summary": "Show flow",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "Flow ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ResponseFlow"
                        }
                    },
                    "400": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "401": {
                        "description": "Unauthorized"
                    },
                    "404": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "500": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    }
                }
            },
            "put": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "description": "Update flow by id",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "flow"
                ],
                "summary": "Update flow",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "Flow ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "Update flow",
                        "name": "flow",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.UpdateFlowModel"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ResponseFlow"
                        }
                    },
                    "400": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "401": {
                        "description": "Unauthorized"
                    },
                    "404": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "500": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    }
                }
            },
            "delete": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "description": "Delete flow by id",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "flow"
                ],
                "summary": "Delete flow",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "Flow ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ResponseSuccess"
                        }
                    },
                    "400": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "401": {
                        "description": "Unauthorized"
                    },
                    "404": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "500": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    }
                }
            }
        },
        "/flow/{id}/redactor": {
            "get": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "description": "Get flow by id",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "flow"
                ],
                "summary": "Show flow",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "Flow ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ResponseRedactorFlowModel"
                        }
                    },
                    "400": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "401": {
                        "description": "Unauthorized"
                    },
                    "404": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "500": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    }
                }
            },
            "put": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "description": "Update flow by id",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "flow"
                ],
                "summary": "Update flow",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "Flow ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "Update flow",
                        "name": "flow",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.RedactorFlowModel"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ResponseRedactorFlowModel"
                        }
                    },
                    "400": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "401": {
                        "description": "Unauthorized"
                    },
                    "404": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "500": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    }
                }
            }
        },
        "/flows": {
            "get": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "description": "Get flow list",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "flow"
                ],
                "summary": "Flow list",
                "parameters": [
                    {
                        "type": "integer",
                        "default": 10,
                        "description": "limit",
                        "name": "limit",
                        "in": "query",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "default": 0,
                        "description": "offset",
                        "name": "offset",
                        "in": "query",
                        "required": true
                    },
                    {
                        "type": "string",
                        "default": "DESC",
                        "description": "order",
                        "name": "order",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "default": "id",
                        "description": "sort_by",
                        "name": "sort_by",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ResponseFlowList"
                        }
                    },
                    "400": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "401": {
                        "description": "Unauthorized"
                    },
                    "404": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "500": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    }
                }
            }
        },
        "/flows/search": {
            "get": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "description": "Search flow by name",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "flow"
                ],
                "summary": "Search flow",
                "parameters": [
                    {
                        "type": "string",
                        "description": "query",
                        "name": "query",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "default": 10,
                        "description": "limit",
                        "name": "limit",
                        "in": "query",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "default": 0,
                        "description": "offset",
                        "name": "offset",
                        "in": "query",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ResponseSearchFlow"
                        }
                    },
                    "401": {
                        "description": "Unauthorized"
                    },
                    "500": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    }
                }
            }
        },
        "/image": {
            "post": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "image"
                ],
                "summary": "Add new image",
                "parameters": [
                    {
                        "description": "image params",
                        "name": "image",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.NewImage"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.Image"
                        }
                    },
                    "400": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "500": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    }
                }
            }
        },
        "/image/{id}": {
            "get": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "description": "Get image by id",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "image"
                ],
                "summary": "Show image",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "Image ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.Image"
                        }
                    },
                    "400": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "404": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "500": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    }
                }
            },
            "put": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "description": "Update image by id",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "image"
                ],
                "summary": "Update image",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "Image ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "Update image",
                        "name": "image",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.UpdateImage"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ResponseSuccess"
                        }
                    },
                    "400": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "404": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "500": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    }
                }
            },
            "delete": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "description": "Delete image by id",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "image"
                ],
                "summary": "Delete image",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "Image ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ResponseSuccess"
                        }
                    },
                    "400": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "404": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "500": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    }
                }
            }
        },
        "/images": {
            "get": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "description": "Get image list",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "image"
                ],
                "summary": "Image list",
                "parameters": [
                    {
                        "type": "integer",
                        "default": 10,
                        "description": "limit",
                        "name": "limit",
                        "in": "query",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "default": 0,
                        "description": "offset",
                        "name": "offset",
                        "in": "query",
                        "required": true
                    },
                    {
                        "type": "string",
                        "default": "DESC",
                        "description": "order",
                        "name": "order",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "default": "id",
                        "description": "sort_by",
                        "name": "sort_by",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ImageListModel"
                        }
                    },
                    "400": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "404": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "500": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    }
                }
            }
        },
        "/log": {
            "post": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "log"
                ],
                "summary": "Add new log",
                "parameters": [
                    {
                        "description": "log params",
                        "name": "log",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.NewLogModel"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.NewObjectSuccess"
                        }
                    },
                    "400": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "401": {
                        "description": "Unauthorized"
                    },
                    "500": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    }
                }
            }
        },
        "/log/{id}": {
            "get": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "description": "Get log by id",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "log"
                ],
                "summary": "Show log",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "Log ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ResponseLog"
                        }
                    },
                    "400": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "401": {
                        "description": "Unauthorized"
                    },
                    "404": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "500": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    }
                }
            },
            "delete": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "description": "Delete log by id",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "log"
                ],
                "summary": "Delete log",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "Log ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ResponseSuccess"
                        }
                    },
                    "400": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "401": {
                        "description": "Unauthorized"
                    },
                    "404": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "500": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    }
                }
            }
        },
        "/logs": {
            "get": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "description": "Get log list",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "log"
                ],
                "summary": "Log list",
                "parameters": [
                    {
                        "type": "integer",
                        "default": 10,
                        "description": "limit",
                        "name": "limit",
                        "in": "query",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "default": 0,
                        "description": "offset",
                        "name": "offset",
                        "in": "query",
                        "required": true
                    },
                    {
                        "type": "string",
                        "default": "DESC",
                        "description": "order",
                        "name": "order",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "default": "id",
                        "description": "sort_by",
                        "name": "sort_by",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "query",
                        "name": "query",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ResponseLogList"
                        }
                    },
                    "400": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "401": {
                        "description": "Unauthorized"
                    },
                    "404": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "500": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    }
                }
            }
        },
        "/logs/search": {
            "get": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "description": "Search log by name",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "log"
                ],
                "summary": "Search log",
                "parameters": [
                    {
                        "type": "string",
                        "description": "query",
                        "name": "query",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "default": 10,
                        "description": "limit",
                        "name": "limit",
                        "in": "query",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "default": 0,
                        "description": "offset",
                        "name": "offset",
                        "in": "query",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ResponseSearchLog"
                        }
                    },
                    "401": {
                        "description": "Unauthorized"
                    },
                    "500": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    }
                }
            }
        },
        "/map": {
            "post": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "map"
                ],
                "summary": "Add new map",
                "parameters": [
                    {
                        "description": "map params",
                        "name": "map",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.NewMap"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.Map"
                        }
                    },
                    "400": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "401": {
                        "description": "Unauthorized"
                    },
                    "500": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    }
                }
            }
        },
        "/map/{id}": {
            "get": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "description": "Get map by id",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "map"
                ],
                "summary": "Show map",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "Map ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.Map"
                        }
                    },
                    "400": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "401": {
                        "description": "Unauthorized"
                    },
                    "404": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "500": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    }
                }
            },
            "put": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "description": "Update map by id",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "map"
                ],
                "summary": "Update map",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "Map ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "Update map",
                        "name": "map",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.UpdateMap"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ResponseSuccess"
                        }
                    },
                    "400": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "401": {
                        "description": "Unauthorized"
                    },
                    "404": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "500": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    }
                }
            },
            "delete": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "description": "Delete map by id",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "map"
                ],
                "summary": "Delete map",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "Map ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ResponseSuccess"
                        }
                    },
                    "400": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "401": {
                        "description": "Unauthorized"
                    },
                    "404": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "500": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    }
                }
            }
        },
        "/map/{id}/full": {
            "get": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "description": "Get map by id",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "map"
                ],
                "summary": "Show full map",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "Map ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.MapFullModel"
                        }
                    },
                    "400": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "401": {
                        "description": "Unauthorized"
                    },
                    "404": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "500": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    }
                }
            }
        },
        "/map_element": {
            "post": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "map_element"
                ],
                "summary": "Add new map_element",
                "parameters": [
                    {
                        "description": "map_element params",
                        "name": "map_element",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.NewMapElement"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.MapElement"
                        }
                    },
                    "400": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "401": {
                        "description": "Unauthorized"
                    },
                    "500": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    }
                }
            }
        },
        "/map_element/{id}": {
            "get": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "description": "Get map_element by id",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "map_element"
                ],
                "summary": "Show map_element",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "MapElement ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.MapElement"
                        }
                    },
                    "400": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "401": {
                        "description": "Unauthorized"
                    },
                    "404": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "500": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    }
                }
            },
            "put": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "description": "Sort map_element by id",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "map_element"
                ],
                "summary": "Sort map_element",
                "parameters": [
                    {
                        "description": "Sort map_element",
                        "name": "map_element",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.SortMapElement"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ResponseSuccess"
                        }
                    },
                    "400": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "401": {
                        "description": "Unauthorized"
                    },
                    "404": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "500": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    }
                }
            },
            "delete": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "description": "Delete map_element by id",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "map_element"
                ],
                "summary": "Delete map_element",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "MapElement ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ResponseSuccess"
                        }
                    },
                    "400": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "401": {
                        "description": "Unauthorized"
                    },
                    "404": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "500": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    }
                }
            }
        },
        "/map_elements": {
            "get": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "description": "Get map_element list",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "map_element"
                ],
                "summary": "MapElement list",
                "parameters": [
                    {
                        "type": "integer",
                        "default": 10,
                        "description": "limit",
                        "name": "limit",
                        "in": "query",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "default": 0,
                        "description": "offset",
                        "name": "offset",
                        "in": "query",
                        "required": true
                    },
                    {
                        "type": "string",
                        "default": "DESC",
                        "description": "order",
                        "name": "order",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "default": "id",
                        "description": "sort_by",
                        "name": "sort_by",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.MapListModel"
                        }
                    },
                    "400": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "401": {
                        "description": "Unauthorized"
                    },
                    "404": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "500": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    }
                }
            }
        },
        "/map_layer": {
            "post": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "map_layer"
                ],
                "summary": "Add new map_layer",
                "parameters": [
                    {
                        "description": "map_layer params",
                        "name": "map_layer",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.NewMapLayer"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.MapLayer"
                        }
                    },
                    "400": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "401": {
                        "description": "Unauthorized"
                    },
                    "500": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    }
                }
            }
        },
        "/map_layer/{id}": {
            "get": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "description": "Get map_layer by id",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "map_layer"
                ],
                "summary": "Show map_layer",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "MapLayer ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.MapLayer"
                        }
                    },
                    "400": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "401": {
                        "description": "Unauthorized"
                    },
                    "404": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "500": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    }
                }
            },
            "put": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "description": "Sort map_layer by id",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "map_layer"
                ],
                "summary": "Sort map_layer",
                "parameters": [
                    {
                        "description": "Sort map_layer",
                        "name": "map_layer",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.SortMapLayer"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ResponseSuccess"
                        }
                    },
                    "400": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "401": {
                        "description": "Unauthorized"
                    },
                    "404": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "500": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    }
                }
            },
            "delete": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "description": "Delete map_layer by id",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "map_layer"
                ],
                "summary": "Delete map_layer",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "MapLayer ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ResponseSuccess"
                        }
                    },
                    "400": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "401": {
                        "description": "Unauthorized"
                    },
                    "404": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "500": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    }
                }
            }
        },
        "/map_layers": {
            "get": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "description": "Get map_layer list",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "map_layer"
                ],
                "summary": "MapLayer list",
                "parameters": [
                    {
                        "type": "integer",
                        "default": 10,
                        "description": "limit",
                        "name": "limit",
                        "in": "query",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "default": 0,
                        "description": "offset",
                        "name": "offset",
                        "in": "query",
                        "required": true
                    },
                    {
                        "type": "string",
                        "default": "DESC",
                        "description": "order",
                        "name": "order",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "default": "id",
                        "description": "sort_by",
                        "name": "sort_by",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.MapListModel"
                        }
                    },
                    "400": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "401": {
                        "description": "Unauthorized"
                    },
                    "404": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "500": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    }
                }
            }
        },
        "/maps": {
            "get": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "description": "Get map list",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "map"
                ],
                "summary": "Map list",
                "parameters": [
                    {
                        "type": "integer",
                        "default": 10,
                        "description": "limit",
                        "name": "limit",
                        "in": "query",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "default": 0,
                        "description": "offset",
                        "name": "offset",
                        "in": "query",
                        "required": true
                    },
                    {
                        "type": "string",
                        "default": "DESC",
                        "description": "order",
                        "name": "order",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "default": "id",
                        "description": "sort_by",
                        "name": "sort_by",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.MapListModel"
                        }
                    },
                    "400": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "401": {
                        "description": "Unauthorized"
                    },
                    "404": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "500": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    }
                }
            }
        },
        "/maps/search": {
            "get": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "description": "Search map by name",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "map"
                ],
                "summary": "Search map",
                "parameters": [
                    {
                        "type": "string",
                        "description": "query",
                        "name": "query",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "default": 10,
                        "description": "limit",
                        "name": "limit",
                        "in": "query",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "default": 0,
                        "description": "offset",
                        "name": "offset",
                        "in": "query",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.SearchMapResponse"
                        }
                    },
                    "400": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "401": {
                        "description": "Unauthorized"
                    },
                    "404": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "500": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    }
                }
            }
        },
        "/node": {
            "post": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "node"
                ],
                "summary": "Add new node",
                "parameters": [
                    {
                        "description": "node params",
                        "name": "node",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.NewNode"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.NewObjectSuccess"
                        }
                    },
                    "400": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "401": {
                        "description": "Unauthorized"
                    },
                    "500": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    }
                }
            }
        },
        "/node/{id}": {
            "get": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "description": "Get node by id",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "node"
                ],
                "summary": "Show node",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "Node ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ResponseNodeModel"
                        }
                    },
                    "400": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "401": {
                        "description": "Unauthorized"
                    },
                    "404": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "500": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    }
                }
            },
            "put": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "description": "Update node by id",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "node"
                ],
                "summary": "Update node",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "Node ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "Update node",
                        "name": "node",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.UpdateNodeModel"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ResponseSuccess"
                        }
                    },
                    "400": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "401": {
                        "description": "Unauthorized"
                    },
                    "404": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "500": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    }
                }
            },
            "delete": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "description": "Delete node by id",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "node"
                ],
                "summary": "Delete node",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "Node ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ResponseSuccess"
                        }
                    },
                    "400": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "401": {
                        "description": "Unauthorized"
                    },
                    "404": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "500": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    }
                }
            }
        },
        "/nodes": {
            "get": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "description": "Get node list",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "node"
                ],
                "summary": "Node list",
                "parameters": [
                    {
                        "type": "integer",
                        "default": 10,
                        "description": "limit",
                        "name": "limit",
                        "in": "query",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "default": 0,
                        "description": "offset",
                        "name": "offset",
                        "in": "query",
                        "required": true
                    },
                    {
                        "type": "string",
                        "default": "DESC",
                        "description": "order",
                        "name": "order",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "default": "id",
                        "description": "sort_by",
                        "name": "sort_by",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.NodeListModel"
                        }
                    },
                    "400": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "401": {
                        "description": "Unauthorized"
                    },
                    "404": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "500": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    }
                }
            }
        },
        "/nodes/search": {
            "get": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "description": "Search node by name",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "node"
                ],
                "summary": "Search node",
                "parameters": [
                    {
                        "type": "string",
                        "description": "query",
                        "name": "query",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "default": 10,
                        "description": "limit",
                        "name": "limit",
                        "in": "query",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "default": 0,
                        "description": "offset",
                        "name": "offset",
                        "in": "query",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ResponseSearchNode"
                        }
                    },
                    "400": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "401": {
                        "description": "Unauthorized"
                    },
                    "404": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "500": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    }
                }
            }
        },
        "/recovery": {
            "post": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "auth"
                ],
                "summary": "Recovery access",
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ResponseSuccess"
                        }
                    },
                    "400": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "500": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    }
                }
            }
        },
        "/reset": {
            "post": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "auth"
                ],
                "summary": "Reset password",
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ResponseSuccess"
                        }
                    },
                    "400": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "500": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    }
                }
            }
        },
        "/role": {
            "post": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "role"
                ],
                "summary": "Add new role",
                "parameters": [
                    {
                        "description": "role params",
                        "name": "role",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.NewRole"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ResponseRoleModel"
                        }
                    },
                    "400": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "401": {
                        "description": "Unauthorized"
                    },
                    "500": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    }
                }
            }
        },
        "/role/{name}": {
            "get": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "description": "Get role by name",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "role"
                ],
                "summary": "Show role",
                "parameters": [
                    {
                        "type": "string",
                        "description": "RoleModel name",
                        "name": "name",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ResponseRoleModel"
                        }
                    },
                    "400": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "401": {
                        "description": "Unauthorized"
                    },
                    "404": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "500": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    }
                }
            },
            "put": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "description": "Update role by name",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "role"
                ],
                "summary": "Update role",
                "parameters": [
                    {
                        "type": "string",
                        "description": "RoleModel name",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "Update role",
                        "name": "role",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.UpdateRole"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ResponseSuccess"
                        }
                    },
                    "400": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "401": {
                        "description": "Unauthorized"
                    },
                    "404": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "500": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    }
                }
            },
            "delete": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "description": "Delete role by name",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "role"
                ],
                "summary": "Delete role",
                "parameters": [
                    {
                        "type": "string",
                        "description": "RoleModel name",
                        "name": "name",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ResponseSuccess"
                        }
                    },
                    "400": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "401": {
                        "description": "Unauthorized"
                    },
                    "404": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "500": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    }
                }
            }
        },
        "/role/{name}/access_list": {
            "get": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "description": "Get access list",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "role"
                ],
                "summary": "get role access list",
                "parameters": [
                    {
                        "type": "string",
                        "description": "RoleModel name",
                        "name": "name",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ResponseAccessList"
                        }
                    },
                    "400": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "401": {
                        "description": "Unauthorized"
                    },
                    "404": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "500": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    }
                }
            },
            "put": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "description": "Update access list",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "role"
                ],
                "summary": "update role access list",
                "parameters": [
                    {
                        "type": "string",
                        "description": "RoleModel name",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "permission",
                        "name": "diff",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.AccessListDiff"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ResponseSuccess"
                        }
                    },
                    "400": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "401": {
                        "description": "Unauthorized"
                    },
                    "404": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "500": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    }
                }
            }
        },
        "/roles": {
            "get": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "description": "Get role list",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "role"
                ],
                "summary": "RoleModel list",
                "parameters": [
                    {
                        "type": "integer",
                        "default": 10,
                        "description": "limit",
                        "name": "limit",
                        "in": "query",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "default": 0,
                        "description": "offset",
                        "name": "offset",
                        "in": "query",
                        "required": true
                    },
                    {
                        "type": "string",
                        "default": "DESC",
                        "description": "order",
                        "name": "order",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "default": "name",
                        "description": "sort_by",
                        "name": "sort_by",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.RoleListModel"
                        }
                    },
                    "400": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "401": {
                        "description": "Unauthorized"
                    },
                    "404": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "500": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    }
                }
            }
        },
        "/roles/search": {
            "get": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "description": "Search role by name",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "role"
                ],
                "summary": "Search role",
                "parameters": [
                    {
                        "type": "string",
                        "description": "query",
                        "name": "query",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "default": 10,
                        "description": "limit",
                        "name": "limit",
                        "in": "query",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "default": 0,
                        "description": "offset",
                        "name": "offset",
                        "in": "query",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.SearchRoleResponse"
                        }
                    },
                    "400": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "401": {
                        "description": "Unauthorized"
                    },
                    "404": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "500": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    }
                }
            }
        },
        "/script": {
            "post": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "script"
                ],
                "summary": "Add new script",
                "parameters": [
                    {
                        "description": "script params",
                        "name": "script",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.NewScript"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.NewObjectSuccess"
                        }
                    },
                    "400": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "500": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    }
                }
            }
        },
        "/script/{id}": {
            "get": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "description": "Get script by id",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "script"
                ],
                "summary": "Show script",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "Script ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ResponseScript"
                        }
                    },
                    "400": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "404": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "500": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    }
                }
            },
            "put": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "description": "Update script by id",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "script"
                ],
                "summary": "Update script",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "Script ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "Update script",
                        "name": "script",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.UpdateScript"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ResponseSuccess"
                        }
                    },
                    "400": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "404": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "500": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    }
                }
            },
            "delete": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "description": "Delete script by id",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "script"
                ],
                "summary": "Delete script",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "Script ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ResponseSuccess"
                        }
                    },
                    "400": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "404": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "500": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    }
                }
            }
        },
        "/script/{id}/exec": {
            "post": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "description": "Execute script by id",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "script"
                ],
                "summary": "Execute script",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "Script ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ResponseScriptExec"
                        }
                    },
                    "400": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "404": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "500": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    }
                }
            }
        },
        "/script/{id}/exec_src": {
            "post": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "description": "Execute script by id",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "script"
                ],
                "summary": "Execute script",
                "parameters": [
                    {
                        "description": "Exec script",
                        "name": "script",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ExecScript"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ResponseScriptExec"
                        }
                    },
                    "400": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "404": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "500": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    }
                }
            }
        },
        "/scripts": {
            "get": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "description": "Get script list",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "script"
                ],
                "summary": "Script list",
                "parameters": [
                    {
                        "type": "integer",
                        "default": 10,
                        "description": "limit",
                        "name": "limit",
                        "in": "query",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "default": 0,
                        "description": "offset",
                        "name": "offset",
                        "in": "query",
                        "required": true
                    },
                    {
                        "type": "string",
                        "default": "DESC",
                        "description": "order",
                        "name": "order",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "default": "id",
                        "description": "sort_by",
                        "name": "sort_by",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ResponseScriptList"
                        }
                    },
                    "400": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "404": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "500": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    }
                }
            }
        },
        "/scripts/search": {
            "get": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "description": "Search device by name",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "script"
                ],
                "summary": "Search device",
                "parameters": [
                    {
                        "type": "string",
                        "description": "query",
                        "name": "query",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "default": 10,
                        "description": "limit",
                        "name": "limit",
                        "in": "query",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "default": 0,
                        "description": "offset",
                        "name": "offset",
                        "in": "query",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.SearchScriptResponse"
                        }
                    },
                    "400": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "404": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "500": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    }
                }
            }
        },
        "/signin": {
            "post": {
                "security": [
                    {
                        "BasicAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "auth"
                ],
                "summary": "Add new auth",
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.AuthSignInResponse"
                        }
                    },
                    "400": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "401": {
                        "description": "user not found"
                    },
                    "403": {
                        "description": "password not valid"
                    },
                    "500": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    }
                }
            }
        },
        "/signout": {
            "post": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "auth"
                ],
                "summary": "Sign out",
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ResponseSuccess"
                        }
                    },
                    "403": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "500": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    }
                }
            }
        },
        "/user": {
            "post": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "user"
                ],
                "summary": "Add new user",
                "parameters": [
                    {
                        "description": "user params",
                        "name": "user",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.NewUser"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.UserFullModel"
                        }
                    },
                    "400": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "500": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    }
                }
            }
        },
        "/user/{id}": {
            "get": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "description": "Get user by id",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "user"
                ],
                "summary": "Show user",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "User ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.UserFullModel"
                        }
                    },
                    "400": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "404": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "500": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    }
                }
            },
            "put": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "description": "Update user by id",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "user"
                ],
                "summary": "Update user",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "User ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "Update user",
                        "name": "user",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.UpdateUser"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.UserFullModel"
                        }
                    },
                    "400": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "404": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "500": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    }
                }
            },
            "delete": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "description": "Delete user by id",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "user"
                ],
                "summary": "Delete user",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "User ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ResponseSuccess"
                        }
                    },
                    "400": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "404": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "500": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    }
                }
            }
        },
        "/user/{id}/update_status": {
            "put": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "description": "Update user status",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "user"
                ],
                "summary": "Update user status",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "User ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "Update user",
                        "name": "user",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.UserUpdateStatusRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ResponseSuccess"
                        }
                    },
                    "400": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "404": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "500": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    }
                }
            }
        },
        "/users": {
            "get": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "description": "Get user list",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "user"
                ],
                "summary": "User list",
                "parameters": [
                    {
                        "type": "integer",
                        "default": 10,
                        "description": "limit",
                        "name": "limit",
                        "in": "query",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "default": 0,
                        "description": "offset",
                        "name": "offset",
                        "in": "query",
                        "required": true
                    },
                    {
                        "type": "string",
                        "default": "DESC",
                        "description": "order",
                        "name": "order",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "default": "id",
                        "description": "sort_by",
                        "name": "sort_by",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.UserListModel"
                        }
                    },
                    "400": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "404": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "500": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    }
                }
            }
        },
        "/workflow": {
            "get": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "description": "Get workflow list",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "workflow"
                ],
                "summary": "Workflow list",
                "parameters": [
                    {
                        "type": "integer",
                        "default": 10,
                        "description": "limit",
                        "name": "limit",
                        "in": "query",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "default": 0,
                        "description": "offset",
                        "name": "offset",
                        "in": "query",
                        "required": true
                    },
                    {
                        "type": "string",
                        "default": "DESC",
                        "description": "order",
                        "name": "order",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "default": "id",
                        "description": "sort_by",
                        "name": "sort_by",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ResponseWorkflowList"
                        }
                    },
                    "400": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "404": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "500": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    }
                }
            },
            "post": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "workflow"
                ],
                "summary": "Add new workflow",
                "parameters": [
                    {
                        "description": "workflow params",
                        "name": "workflow",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.NewWorkflow"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.NewObjectSuccess"
                        }
                    },
                    "400": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "500": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    }
                }
            }
        },
        "/workflow/{id}": {
            "get": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "description": "Get workflow by id",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "workflow"
                ],
                "summary": "Show workflow",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "Workflow ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ResponseWorkflowModel"
                        }
                    },
                    "400": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "404": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "500": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    }
                }
            },
            "put": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "description": "Update workflow by id",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "workflow"
                ],
                "summary": "Update workflow",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "Workflow ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "Update workflow",
                        "name": "workflow",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.UpdateWorkflow"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ResponseSuccess"
                        }
                    },
                    "400": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "404": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "500": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    }
                }
            },
            "delete": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "description": "Delete workflow by id",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "workflow"
                ],
                "summary": "Delete workflow",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "Workflow ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ResponseSuccess"
                        }
                    },
                    "400": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "404": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "500": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    }
                }
            }
        },
        "/workflow/{id}/scenario": {
            "post": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "workflow_scenario"
                ],
                "summary": "Add new workflow_scenario",
                "parameters": [
                    {
                        "description": "workflow_scenario params",
                        "name": "workflow_scenario",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.NewWorkflowScenario"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.NewObjectSuccess"
                        }
                    },
                    "400": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "500": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    }
                }
            }
        },
        "/workflow/{id}/scenario/{scenario_id}": {
            "get": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "description": "Get workflow_scenario by id",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "workflow_scenario"
                ],
                "summary": "Show workflow_scenario",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "WorkflowScenario ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.WorkflowScenario"
                        }
                    },
                    "400": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "404": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "500": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    }
                }
            },
            "put": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "description": "Update workflow_scenario by id",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "workflow_scenario"
                ],
                "summary": "Update workflow_scenario",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "WorkflowScenario ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "Update workflow_scenario",
                        "name": "workflow_scenario",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.WorkflowUpdateWorkflowScenario"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ResponseSuccess"
                        }
                    },
                    "400": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "404": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "500": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    }
                }
            },
            "delete": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "description": "Delete workflow_scenario by id",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "workflow_scenario"
                ],
                "summary": "Delete workflow_scenario",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "WorkflowScenario ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ResponseSuccess"
                        }
                    },
                    "400": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "404": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "500": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    }
                }
            }
        },
        "/workflow/{id}/scenarios": {
            "get": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "description": "Get workflow_scenario list",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "workflow_scenario"
                ],
                "summary": "WorkflowScenario list",
                "parameters": [
                    {
                        "type": "integer",
                        "default": 10,
                        "description": "limit",
                        "name": "limit",
                        "in": "query",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "default": 0,
                        "description": "offset",
                        "name": "offset",
                        "in": "query",
                        "required": true
                    },
                    {
                        "type": "string",
                        "default": "DESC",
                        "description": "order",
                        "name": "order",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "default": "id",
                        "description": "sort_by",
                        "name": "sort_by",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.WorkflowScenarioListModel"
                        }
                    },
                    "400": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "404": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "500": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    }
                }
            }
        },
        "/workflow/{id}/scenarios/search": {
            "get": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "description": "Search workflow by name",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "workflow"
                ],
                "summary": "Search workflow",
                "parameters": [
                    {
                        "type": "string",
                        "description": "query",
                        "name": "query",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "default": 10,
                        "description": "limit",
                        "name": "limit",
                        "in": "query",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "default": 0,
                        "description": "offset",
                        "name": "offset",
                        "in": "query",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.SearchWorkflowScenarioResponse"
                        }
                    },
                    "400": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "404": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "500": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    }
                }
            }
        },
        "/workflow/{id}/update_scenario": {
            "put": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "description": "Update workflow by id",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "workflow"
                ],
                "summary": "Update workflow scenario",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "Workflow ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "Update workflow scenario",
                        "name": "scenario",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.WorkflowUpdateWorkflowScenario"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ResponseSuccess"
                        }
                    },
                    "400": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "404": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "500": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    }
                }
            }
        },
        "/workflows/search": {
            "get": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "description": "Search workflow by name",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "workflow"
                ],
                "summary": "Search workflow",
                "parameters": [
                    {
                        "type": "string",
                        "description": "query",
                        "name": "query",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "default": 10,
                        "description": "limit",
                        "name": "limit",
                        "in": "query",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "default": 0,
                        "description": "offset",
                        "name": "offset",
                        "in": "query",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.SearchWorkflowResponse"
                        }
                    },
                    "400": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "404": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    },
                    "500": {
                        "description": "some error",
                        "schema": {
                            "type": "object",
                            "$ref": "#/definitions/models.ErrorModel"
                        }
                    }
                }
            }
        }
    },
    "definitions": {
        "models.AccessListDiff": {
            "type": "object"
        },
        "models.AuthSignInResponse": {
            "type": "object",
            "properties": {
                "access_token": {
                    "type": "string"
                },
                "current_user": {
                    "type": "object",
                    "$ref": "#/definitions/models.CurrentUserModel"
                }
            }
        },
        "models.CurrentUserModel": {
            "type": "object",
            "properties": {
                "email": {
                    "type": "string"
                },
                "first_name": {
                    "type": "string"
                },
                "history": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/models.UserHistory"
                    }
                },
                "id": {
                    "type": "integer"
                },
                "image": {
                    "type": "object",
                    "$ref": "#/definitions/models.Image"
                },
                "lang": {
                    "type": "string"
                },
                "last_name": {
                    "type": "string"
                },
                "meta": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/models.UserByIdModelMeta"
                    }
                },
                "nickname": {
                    "type": "string"
                },
                "role": {
                    "type": "object",
                    "$ref": "#/definitions/models.RoleModel"
                },
                "sign_in_count": {
                    "type": "integer"
                }
            }
        },
        "models.DeviceAction": {
            "type": "object",
            "properties": {
                "created_at": {
                    "type": "string"
                },
                "description": {
                    "type": "string"
                },
                "device": {
                    "type": "object",
                    "$ref": "#/definitions/models.DeviceActionDevice"
                },
                "id": {
                    "type": "integer"
                },
                "name": {
                    "type": "string"
                },
                "script": {
                    "type": "object",
                    "$ref": "#/definitions/models.DeviceActionScript"
                },
                "updated_at": {
                    "type": "string"
                }
            }
        },
        "models.DeviceActionDevice": {
            "type": "object",
            "properties": {
                "id": {
                    "type": "integer"
                }
            }
        },
        "models.DeviceActionScript": {
            "type": "object",
            "properties": {
                "id": {
                    "type": "integer"
                }
            }
        },
        "models.DeviceListModel": {
            "type": "object",
            "properties": {
                "items": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/models.DeviceModel"
                    }
                },
                "meta": {
                    "type": "object",
                    "properties": {
                        "limit": {
                            "type": "integer"
                        },
                        "objects_count": {
                            "type": "integer"
                        },
                        "offset": {
                            "type": "integer"
                        }
                    }
                }
            }
        },
        "models.DeviceModel": {
            "type": "object",
            "properties": {
                "actions": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/models.DeviceAction"
                    }
                },
                "created_at": {
                    "type": "string"
                },
                "description": {
                    "type": "string"
                },
                "device": {
                    "type": "object",
                    "$ref": "#/definitions/models.ParentDevice"
                },
                "device_id": {
                    "type": "integer"
                },
                "id": {
                    "type": "integer"
                },
                "is_group": {
                    "type": "boolean"
                },
                "name": {
                    "type": "string"
                },
                "node": {
                    "type": "object",
                    "$ref": "#/definitions/models.NodeModel"
                },
                "properties": {
                    "type": "object"
                },
                "states": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/models.DeviceState"
                    }
                },
                "status": {
                    "type": "string"
                },
                "type": {
                    "type": "string"
                },
                "updated_at": {
                    "type": "string"
                }
            }
        },
        "models.DeviceState": {
            "type": "object",
            "properties": {
                "created_at": {
                    "type": "string"
                },
                "description": {
                    "type": "string"
                },
                "device": {
                    "type": "object",
                    "$ref": "#/definitions/models.DeviceStateDevice"
                },
                "id": {
                    "type": "integer"
                },
                "system_name": {
                    "type": "string"
                },
                "updated_at": {
                    "type": "string"
                }
            }
        },
        "models.DeviceStateDevice": {
            "type": "object",
            "properties": {
                "id": {
                    "type": "integer"
                }
            }
        },
        "models.ErrorModel": {
            "type": "object",
            "properties": {
                "code": {
                    "description": "code",
                    "type": "string"
                },
                "errors": {
                    "description": "errors",
                    "type": "object",
                    "$ref": "#/definitions/models.ErrorModelErrors"
                },
                "message": {
                    "description": "описание ошибки",
                    "type": "string"
                }
            }
        },
        "models.ErrorModelErrors": {
            "type": "object"
        },
        "models.ExecScript": {
            "type": "object",
            "properties": {
                "description": {
                    "type": "string"
                },
                "lang": {
                    "type": "string"
                },
                "name": {
                    "type": "string"
                },
                "source": {
                    "type": "string"
                }
            }
        },
        "models.FlowConnectionModel": {
            "type": "object",
            "properties": {
                "created_at": {
                    "type": "string"
                },
                "direction": {
                    "type": "string"
                },
                "element_from": {
                    "type": "string"
                },
                "element_to": {
                    "type": "string"
                },
                "flow_id": {
                    "type": "integer"
                },
                "graph_settings": {
                    "type": "string"
                },
                "name": {
                    "type": "string"
                },
                "point_from": {
                    "type": "integer"
                },
                "point_to": {
                    "type": "integer"
                },
                "updated_at": {
                    "type": "string"
                },
                "uuid": {
                    "type": "string"
                }
            }
        },
        "models.FlowElementModel": {
            "type": "object",
            "properties": {
                "created_at": {
                    "type": "string"
                },
                "description": {
                    "type": "string"
                },
                "flow_id": {
                    "type": "integer"
                },
                "flow_link": {
                    "type": "integer"
                },
                "graph_settings": {
                    "type": "string"
                },
                "name": {
                    "type": "string"
                },
                "prototype_type": {
                    "type": "string"
                },
                "script": {
                    "type": "object",
                    "$ref": "#/definitions/models.Script"
                },
                "script_id": {
                    "type": "integer"
                },
                "status": {
                    "type": "string"
                },
                "updated_at": {
                    "type": "string"
                },
                "uuid": {
                    "type": "string"
                }
            }
        },
        "models.FlowListModel": {
            "type": "object",
            "properties": {
                "created_at": {
                    "type": "string"
                },
                "description": {
                    "type": "string"
                },
                "id": {
                    "type": "integer"
                },
                "name": {
                    "type": "string"
                },
                "status": {
                    "type": "string"
                },
                "updated_at": {
                    "type": "string"
                },
                "workers": {
                    "type": "array",
                    "items": {
                        "type": "\u0026{%!s(token.Pos=316) %!s(*ast.FieldList=\u0026{323 [0xc0002633c0] 349}) %!s(bool=false)}"
                    }
                },
                "workflow": {
                    "type": "object",
                    "properties": {
                        "id": {
                            "type": "integer"
                        },
                        "name": {
                            "type": "string"
                        }
                    }
                }
            }
        },
        "models.FlowModel": {
            "type": "object",
            "properties": {
                "connections": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/models.FlowConnectionModel"
                    }
                },
                "created_at": {
                    "type": "string"
                },
                "description": {
                    "type": "string"
                },
                "flow_elements": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/models.FlowElementModel"
                    }
                },
                "id": {
                    "type": "integer"
                },
                "name": {
                    "type": "string"
                },
                "status": {
                    "type": "string"
                },
                "updated_at": {
                    "type": "string"
                },
                "workers": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/models.FlowWorkerModel"
                    }
                },
                "workflow": {
                    "type": "object",
                    "$ref": "#/definitions/models.FlowWorkflowModel"
                },
                "workflow_id": {
                    "type": "integer"
                },
                "workflow_scenario_id": {
                    "type": "integer"
                }
            }
        },
        "models.FlowWorkerModel": {
            "type": "object",
            "properties": {
                "created_at": {
                    "type": "string"
                },
                "device_action": {
                    "type": "object",
                    "$ref": "#/definitions/models.DeviceAction"
                },
                "device_action_id": {
                    "type": "integer"
                },
                "flow_id": {
                    "type": "integer"
                },
                "id": {
                    "type": "integer"
                },
                "name": {
                    "type": "string"
                },
                "status": {
                    "type": "string"
                },
                "time": {
                    "type": "string"
                },
                "updated_at": {
                    "type": "string"
                },
                "workflow": {
                    "type": "object",
                    "$ref": "#/definitions/models.FlowWorkflowModel"
                },
                "workflow_id": {
                    "type": "integer"
                }
            }
        },
        "models.FlowWorkflowModel": {
            "type": "object",
            "properties": {
                "created_at": {
                    "type": "string"
                },
                "description": {
                    "type": "string"
                },
                "id": {
                    "type": "integer"
                },
                "name": {
                    "type": "string"
                },
                "status": {
                    "type": "string"
                },
                "updated_at": {
                    "type": "string"
                }
            }
        },
        "models.Image": {
            "type": "object",
            "properties": {
                "created_at": {
                    "type": "string"
                },
                "id": {
                    "type": "integer"
                },
                "image": {
                    "type": "string"
                },
                "mime_type": {
                    "type": "string"
                },
                "name": {
                    "type": "string"
                },
                "size": {
                    "type": "integer"
                },
                "thumb": {
                    "type": "string"
                },
                "title": {
                    "type": "string"
                },
                "url": {
                    "type": "string"
                }
            }
        },
        "models.ImageListModel": {
            "type": "object",
            "properties": {
                "items": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/models.Image"
                    }
                },
                "meta": {
                    "type": "object",
                    "properties": {
                        "limit": {
                            "type": "integer"
                        },
                        "objects_count": {
                            "type": "integer"
                        },
                        "offset": {
                            "type": "integer"
                        }
                    }
                }
            }
        },
        "models.LogModel": {
            "type": "object",
            "properties": {
                "body": {
                    "type": "string"
                },
                "created_at": {
                    "type": "string"
                },
                "id": {
                    "type": "integer"
                },
                "level": {
                    "type": "string"
                }
            }
        },
        "models.Map": {
            "type": "object",
            "properties": {
                "created_at": {
                    "type": "string"
                },
                "description": {
                    "type": "string"
                },
                "id": {
                    "type": "integer"
                },
                "name": {
                    "type": "string"
                },
                "options": {
                    "type": "object",
                    "$ref": "#/definitions/models.MapOptions"
                },
                "updated_at": {
                    "type": "string"
                }
            }
        },
        "models.MapDeviceAction": {
            "type": "object",
            "properties": {
                "created_at": {
                    "type": "string"
                },
                "device_action": {
                    "type": "object",
                    "$ref": "#/definitions/models.DeviceAction"
                },
                "device_action_id": {
                    "type": "integer"
                },
                "id": {
                    "type": "integer"
                },
                "image": {
                    "type": "object",
                    "$ref": "#/definitions/models.Image"
                },
                "image_id": {
                    "type": "integer"
                },
                "map_device_id": {
                    "type": "integer"
                },
                "type": {
                    "type": "string"
                },
                "updated_at": {
                    "type": "string"
                }
            }
        },
        "models.MapDeviceState": {
            "type": "object",
            "properties": {
                "created_at": {
                    "type": "string"
                },
                "device_state": {
                    "type": "object",
                    "$ref": "#/definitions/models.DeviceState"
                },
                "device_state_id": {
                    "type": "integer"
                },
                "id": {
                    "type": "integer"
                },
                "image": {
                    "type": "object",
                    "$ref": "#/definitions/models.Image"
                },
                "image_id": {
                    "type": "integer"
                },
                "map_device_id": {
                    "type": "integer"
                },
                "style": {
                    "type": "string"
                },
                "updated_at": {
                    "type": "string"
                }
            }
        },
        "models.MapElement": {
            "type": "object",
            "properties": {
                "created_at": {
                    "type": "string"
                },
                "description": {
                    "type": "string"
                },
                "graph_settings": {
                    "type": "object",
                    "$ref": "#/definitions/models.MapElementGraphSettings"
                },
                "id": {
                    "type": "integer"
                },
                "layer_id": {
                    "type": "integer"
                },
                "map_id": {
                    "type": "integer"
                },
                "name": {
                    "type": "string"
                },
                "prototype": {
                    "type": "object",
                    "$ref": "#/definitions/models.Prototype"
                },
                "prototype_id": {
                    "type": "integer"
                },
                "prototype_type": {
                    "type": "string"
                },
                "status": {
                    "type": "string"
                },
                "updated_at": {
                    "type": "string"
                },
                "weight": {
                    "type": "integer"
                }
            }
        },
        "models.MapElementGraphSettings": {
            "type": "object",
            "properties": {
                "height": {
                    "type": "integer"
                },
                "position": {
                    "type": "object",
                    "$ref": "#/definitions/models.MapElementGraphSettingsPosition"
                },
                "width": {
                    "type": "integer"
                }
            }
        },
        "models.MapElementGraphSettingsPosition": {
            "type": "object",
            "properties": {
                "left": {
                    "type": "integer"
                },
                "top": {
                    "type": "integer"
                }
            }
        },
        "models.MapFullModel": {
            "type": "object",
            "properties": {
                "created_at": {
                    "type": "string"
                },
                "description": {
                    "type": "string"
                },
                "id": {
                    "type": "integer"
                },
                "layers": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/models.MapLayer"
                    }
                },
                "name": {
                    "type": "string"
                },
                "options": {
                    "type": "object",
                    "$ref": "#/definitions/models.MapOptions"
                },
                "updated_at": {
                    "type": "string"
                }
            }
        },
        "models.MapLayer": {
            "type": "object",
            "properties": {
                "created_at": {
                    "type": "string"
                },
                "description": {
                    "type": "string"
                },
                "elements": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/models.MapElement"
                    }
                },
                "id": {
                    "type": "integer"
                },
                "map": {
                    "type": "object",
                    "$ref": "#/definitions/models.Map"
                },
                "map_id": {
                    "type": "integer"
                },
                "name": {
                    "type": "string"
                },
                "status": {
                    "type": "string"
                },
                "updated_at": {
                    "type": "string"
                },
                "weight": {
                    "type": "integer"
                }
            }
        },
        "models.MapListModel": {
            "type": "object",
            "properties": {
                "items": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/models.Map"
                    }
                },
                "meta": {
                    "type": "object",
                    "properties": {
                        "limit": {
                            "type": "integer"
                        },
                        "objects_count": {
                            "type": "integer"
                        },
                        "offset": {
                            "type": "integer"
                        }
                    }
                }
            }
        },
        "models.MapOptions": {
            "type": "object",
            "properties": {
                "element_option_text": {
                    "type": "boolean"
                },
                "element_state_text": {
                    "type": "boolean"
                },
                "zoom": {
                    "type": "number"
                }
            }
        },
        "models.NewDeviceAction": {
            "type": "object",
            "properties": {
                "description": {
                    "type": "string"
                },
                "device": {
                    "type": "object",
                    "$ref": "#/definitions/models.DeviceActionDevice"
                },
                "name": {
                    "type": "string"
                },
                "script": {
                    "type": "object",
                    "$ref": "#/definitions/models.DeviceActionScript"
                }
            }
        },
        "models.NewDeviceModel": {
            "type": "object",
            "properties": {
                "description": {
                    "type": "string"
                },
                "device": {
                    "type": "object",
                    "$ref": "#/definitions/models.ParentDevice"
                },
                "name": {
                    "type": "string"
                },
                "node": {
                    "type": "object",
                    "$ref": "#/definitions/models.NewDeviceNode"
                },
                "properties": {
                    "type": "object"
                },
                "status": {
                    "type": "string"
                },
                "type": {
                    "type": "string"
                }
            }
        },
        "models.NewDeviceNode": {
            "type": "object",
            "properties": {
                "id": {
                    "type": "integer"
                }
            }
        },
        "models.NewDeviceState": {
            "type": "object",
            "properties": {
                "description": {
                    "type": "string"
                },
                "device": {
                    "type": "object",
                    "$ref": "#/definitions/models.DeviceStateDevice"
                },
                "system_name": {
                    "type": "string"
                }
            }
        },
        "models.NewFlowModel": {
            "type": "object",
            "properties": {
                "description": {
                    "type": "string"
                },
                "name": {
                    "type": "string"
                },
                "scenario": {
                    "type": "object",
                    "properties": {
                        "id": {
                            "type": "integer"
                        }
                    }
                },
                "status": {
                    "type": "string"
                },
                "workflow": {
                    "type": "object",
                    "properties": {
                        "id": {
                            "type": "integer"
                        }
                    }
                }
            }
        },
        "models.NewImage": {
            "type": "object",
            "properties": {
                "image": {
                    "type": "string"
                },
                "mime_type": {
                    "type": "string"
                },
                "name": {
                    "type": "string"
                },
                "size": {
                    "type": "integer"
                },
                "thumb": {
                    "type": "string"
                },
                "title": {
                    "type": "string"
                }
            }
        },
        "models.NewLogModel": {
            "type": "object",
            "properties": {
                "body": {
                    "type": "string"
                },
                "level": {
                    "type": "string"
                }
            }
        },
        "models.NewMap": {
            "type": "object",
            "properties": {
                "description": {
                    "type": "string"
                },
                "name": {
                    "type": "string"
                },
                "options": {
                    "type": "object",
                    "$ref": "#/definitions/models.MapOptions"
                }
            }
        },
        "models.NewMapElement": {
            "type": "object",
            "properties": {
                "description": {
                    "type": "string"
                },
                "graph_settings": {
                    "type": "object",
                    "$ref": "#/definitions/models.MapElementGraphSettings"
                },
                "layer_id": {
                    "type": "integer"
                },
                "map": {
                    "type": "object",
                    "$ref": "#/definitions/models.Map"
                },
                "map_id": {
                    "type": "integer"
                },
                "name": {
                    "type": "string"
                },
                "prototype": {
                    "type": "object",
                    "$ref": "#/definitions/models.Prototype"
                },
                "prototype_id": {
                    "type": "integer"
                },
                "prototype_type": {
                    "type": "string"
                },
                "status": {
                    "type": "string"
                },
                "weight": {
                    "type": "integer"
                }
            }
        },
        "models.NewMapLayer": {
            "type": "object",
            "properties": {
                "description": {
                    "type": "string"
                },
                "map": {
                    "type": "object",
                    "$ref": "#/definitions/models.Map"
                },
                "name": {
                    "type": "string"
                },
                "status": {
                    "type": "string"
                }
            }
        },
        "models.NewNode": {
            "type": "object",
            "properties": {
                "description": {
                    "type": "string"
                },
                "ip": {
                    "type": "string"
                },
                "name": {
                    "type": "string"
                },
                "port": {
                    "type": "integer"
                },
                "status": {
                    "type": "string"
                }
            }
        },
        "models.NewObjectSuccess": {
            "type": "object",
            "properties": {
                "id": {
                    "type": "integer"
                }
            }
        },
        "models.NewRole": {
            "type": "object",
            "properties": {
                "description": {
                    "type": "string"
                },
                "name": {
                    "type": "string"
                },
                "parent": {
                    "type": "object",
                    "properties": {
                        "name": {
                            "type": "string"
                        }
                    }
                }
            }
        },
        "models.NewScript": {
            "type": "object",
            "properties": {
                "description": {
                    "type": "string"
                },
                "lang": {
                    "type": "string"
                },
                "name": {
                    "type": "string"
                },
                "source": {
                    "type": "string"
                }
            }
        },
        "models.NewUser": {
            "type": "object",
            "properties": {
                "avatar": {
                    "type": "object",
                    "$ref": "#/definitions/models.NewUserImage"
                },
                "email": {
                    "type": "string"
                },
                "first_name": {
                    "type": "string"
                },
                "lang": {
                    "type": "string"
                },
                "last_name": {
                    "type": "string"
                },
                "meta": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/models.NewUserMeta"
                    }
                },
                "nickname": {
                    "type": "string"
                },
                "password": {
                    "type": "string"
                },
                "password_repeat": {
                    "type": "string"
                },
                "role": {
                    "type": "object",
                    "$ref": "#/definitions/models.NewUserRole"
                },
                "status": {
                    "type": "string"
                }
            }
        },
        "models.NewUserImage": {
            "type": "object",
            "properties": {
                "id": {
                    "type": "integer"
                }
            }
        },
        "models.NewUserMeta": {
            "type": "object",
            "properties": {
                "key": {
                    "type": "string"
                },
                "value": {
                    "type": "string"
                }
            }
        },
        "models.NewUserRole": {
            "type": "object",
            "properties": {
                "name": {
                    "type": "string"
                }
            }
        },
        "models.NewWorkflow": {
            "type": "object",
            "properties": {
                "description": {
                    "type": "string"
                },
                "name": {
                    "type": "string"
                },
                "status": {
                    "type": "string"
                }
            }
        },
        "models.NewWorkflowScenario": {
            "type": "object",
            "properties": {
                "name": {
                    "type": "string"
                },
                "system_name": {
                    "type": "string"
                },
                "workflow_id": {
                    "type": "integer"
                }
            }
        },
        "models.NodeListModel": {
            "type": "object",
            "properties": {
                "items": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/models.NodeModel"
                    }
                },
                "meta": {
                    "type": "object",
                    "properties": {
                        "limit": {
                            "type": "integer"
                        },
                        "objects_count": {
                            "type": "integer"
                        },
                        "offset": {
                            "type": "integer"
                        }
                    }
                }
            }
        },
        "models.NodeModel": {
            "type": "object",
            "properties": {
                "created_at": {
                    "type": "string"
                },
                "description": {
                    "type": "string"
                },
                "id": {
                    "type": "integer"
                },
                "ip": {
                    "type": "string"
                },
                "name": {
                    "type": "string"
                },
                "port": {
                    "type": "integer"
                },
                "status": {
                    "type": "string"
                },
                "updated_at": {
                    "type": "string"
                }
            }
        },
        "models.ParentDevice": {
            "type": "object",
            "properties": {
                "id": {
                    "type": "integer"
                }
            }
        },
        "models.Prototype": {
            "type": "object",
            "properties": {
                "actions": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/models.MapDeviceAction"
                    }
                },
                "created_at": {
                    "type": "string"
                },
                "device": {
                    "type": "object",
                    "$ref": "#/definitions/models.DeviceModel"
                },
                "device_id": {
                    "type": "integer"
                },
                "id": {
                    "type": "integer"
                },
                "image": {
                    "type": "object",
                    "$ref": "#/definitions/models.Image"
                },
                "image_id": {
                    "type": "integer"
                },
                "states": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/models.MapDeviceState"
                    }
                },
                "style": {
                    "type": "string"
                },
                "system_name": {
                    "type": "string"
                },
                "text": {
                    "type": "string"
                },
                "updated_at": {
                    "type": "string"
                }
            }
        },
        "models.RedactorConnector": {
            "type": "object",
            "properties": {
                "direction": {
                    "type": "string"
                },
                "end": {
                    "type": "object",
                    "properties": {
                        "object": {
                            "type": "string"
                        },
                        "point": {
                            "type": "integer"
                        }
                    }
                },
                "flow_type": {
                    "type": "string"
                },
                "id": {
                    "type": "string"
                },
                "start": {
                    "type": "object",
                    "properties": {
                        "object": {
                            "type": "string"
                        },
                        "point": {
                            "type": "integer"
                        }
                    }
                },
                "title": {
                    "type": "string"
                }
            }
        },
        "models.RedactorFlowModel": {
            "type": "object",
            "properties": {
                "connectors": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/models.RedactorConnector"
                    }
                },
                "created_at": {
                    "type": "string"
                },
                "description": {
                    "type": "string"
                },
                "id": {
                    "type": "integer"
                },
                "name": {
                    "type": "string"
                },
                "objects": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/models.RedactorObject"
                    }
                },
                "scenario": {
                    "type": "object",
                    "$ref": "#/definitions/models.WorkflowScenario"
                },
                "status": {
                    "type": "string"
                },
                "update_at": {
                    "type": "string"
                },
                "workers": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/models.WorkerModel"
                    }
                },
                "workflow": {
                    "type": "object",
                    "$ref": "#/definitions/models.RedactorWorkflowModel"
                }
            }
        },
        "models.RedactorObject": {
            "type": "object",
            "properties": {
                "description": {
                    "type": "string"
                },
                "error": {
                    "type": "string"
                },
                "flow_link": {
                    "type": "object",
                    "$ref": "#/definitions/models.FlowModel"
                },
                "id": {
                    "type": "string"
                },
                "position": {
                    "type": "object",
                    "properties": {
                        "left": {
                            "type": "integer"
                        },
                        "top": {
                            "type": "integer"
                        }
                    }
                },
                "prototype_type": {
                    "type": "string"
                },
                "script": {
                    "type": "object",
                    "$ref": "#/definitions/models.Script"
                },
                "status": {
                    "type": "string"
                },
                "title": {
                    "type": "string"
                },
                "type": {
                    "type": "object",
                    "properties": {
                        "action": {
                            "type": "string"
                        },
                        "end": {
                            "type": "object"
                        },
                        "name": {
                            "type": "string"
                        },
                        "start": {
                            "type": "object"
                        },
                        "status": {
                            "type": "string"
                        }
                    }
                }
            }
        },
        "models.RedactorWorkflowModel": {
            "type": "object",
            "properties": {
                "created_at": {
                    "type": "string"
                },
                "description": {
                    "type": "string"
                },
                "id": {
                    "type": "integer"
                },
                "name": {
                    "type": "string"
                },
                "status": {
                    "type": "string"
                },
                "updated_at": {
                    "type": "string"
                }
            }
        },
        "models.ResponseAccessList": {
            "type": "object",
            "properties": {
                "access_list": {
                    "type": "object"
                }
            }
        },
        "models.ResponseFlow": {
            "type": "object",
            "properties": {
                "flow": {
                    "type": "object",
                    "$ref": "#/definitions/models.FlowModel"
                }
            }
        },
        "models.ResponseFlowList": {
            "type": "object",
            "properties": {
                "items": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/models.FlowListModel"
                    }
                },
                "meta": {
                    "type": "object",
                    "properties": {
                        "limit": {
                            "type": "integer"
                        },
                        "objects_count": {
                            "type": "integer"
                        },
                        "offset": {
                            "type": "integer"
                        }
                    }
                }
            }
        },
        "models.ResponseLog": {
            "type": "object",
            "properties": {
                "log": {
                    "type": "object",
                    "$ref": "#/definitions/models.LogModel"
                }
            }
        },
        "models.ResponseLogList": {
            "type": "object",
            "properties": {
                "items": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/models.LogModel"
                    }
                },
                "meta": {
                    "type": "object",
                    "properties": {
                        "limit": {
                            "type": "integer"
                        },
                        "objects_count": {
                            "type": "integer"
                        },
                        "offset": {
                            "type": "integer"
                        }
                    }
                }
            }
        },
        "models.ResponseNodeModel": {
            "type": "object",
            "properties": {
                "node": {
                    "type": "object",
                    "$ref": "#/definitions/models.NodeModel"
                }
            }
        },
        "models.ResponseRedactorFlowModel": {
            "type": "object",
            "properties": {
                "flow": {
                    "type": "object",
                    "$ref": "#/definitions/models.RedactorFlowModel"
                }
            }
        },
        "models.ResponseRoleModel": {
            "type": "object",
            "properties": {
                "role": {
                    "type": "object",
                    "$ref": "#/definitions/models.RoleModel"
                }
            }
        },
        "models.ResponseScript": {
            "type": "object",
            "properties": {
                "code": {
                    "type": "string"
                },
                "data": {
                    "type": "object",
                    "properties": {
                        "script": {
                            "type": "object",
                            "$ref": "#/definitions/models.Script"
                        }
                    }
                }
            }
        },
        "models.ResponseScriptExec": {
            "type": "object",
            "properties": {
                "code": {
                    "type": "string"
                },
                "data": {
                    "type": "object",
                    "properties": {
                        "result": {
                            "type": "string"
                        }
                    }
                }
            }
        },
        "models.ResponseScriptList": {
            "type": "object",
            "properties": {
                "code": {
                    "type": "string"
                },
                "data": {
                    "type": "object",
                    "properties": {
                        "items": {
                            "type": "array",
                            "items": {
                                "$ref": "#/definitions/models.Script"
                            }
                        },
                        "limit": {
                            "type": "integer"
                        },
                        "offset": {
                            "type": "integer"
                        },
                        "total": {
                            "type": "integer"
                        }
                    }
                }
            }
        },
        "models.ResponseSearchFlow": {
            "type": "object",
            "properties": {
                "flows": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/models.FlowModel"
                    }
                }
            }
        },
        "models.ResponseSearchLog": {
            "type": "object",
            "properties": {
                "logs": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/models.LogModel"
                    }
                }
            }
        },
        "models.ResponseSearchNode": {
            "type": "object",
            "properties": {
                "nodes": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/models.NodeModel"
                    }
                }
            }
        },
        "models.ResponseSuccess": {
            "type": "object"
        },
        "models.ResponseWorkflowList": {
            "type": "object",
            "properties": {
                "code": {
                    "type": "string"
                },
                "data": {
                    "type": "object",
                    "properties": {
                        "items": {
                            "type": "array",
                            "items": {
                                "$ref": "#/definitions/models.WorkflowModel"
                            }
                        },
                        "limit": {
                            "type": "integer"
                        },
                        "offset": {
                            "type": "integer"
                        },
                        "total": {
                            "type": "integer"
                        }
                    }
                }
            }
        },
        "models.ResponseWorkflowModel": {
            "type": "object",
            "properties": {
                "code": {
                    "type": "string"
                },
                "data": {
                    "type": "object",
                    "properties": {
                        "workflow": {
                            "type": "object",
                            "$ref": "#/definitions/models.WorkflowModel"
                        }
                    }
                }
            }
        },
        "models.RoleListModel": {
            "type": "object",
            "properties": {
                "items": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/models.RoleModel"
                    }
                },
                "meta": {
                    "type": "object",
                    "properties": {
                        "limit": {
                            "type": "integer"
                        },
                        "objects_count": {
                            "type": "integer"
                        },
                        "offset": {
                            "type": "integer"
                        }
                    }
                }
            }
        },
        "models.RoleModel": {
            "type": "object",
            "properties": {
                "access_list": {
                    "type": "object"
                },
                "children": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/models.RoleModel"
                    }
                },
                "created_at": {
                    "type": "string"
                },
                "description": {
                    "type": "string"
                },
                "name": {
                    "type": "string"
                },
                "parent": {
                    "type": "object",
                    "properties": {
                        "name": {
                            "type": "string"
                        }
                    }
                },
                "updated_at": {
                    "type": "string"
                }
            }
        },
        "models.Script": {
            "type": "object",
            "properties": {
                "created_at": {
                    "type": "string"
                },
                "description": {
                    "type": "string"
                },
                "id": {
                    "type": "integer"
                },
                "lang": {
                    "type": "string"
                },
                "name": {
                    "type": "string"
                },
                "source": {
                    "type": "string"
                },
                "updated_at": {
                    "type": "string"
                }
            }
        },
        "models.SearchDeviceActionResponse": {
            "type": "object",
            "properties": {
                "actions": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/models.DeviceAction"
                    }
                }
            }
        },
        "models.SearchDeviceResponse": {
            "type": "object",
            "properties": {
                "devices": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/models.DeviceModel"
                    }
                }
            }
        },
        "models.SearchMapResponse": {
            "type": "object",
            "properties": {
                "maps": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/models.Map"
                    }
                }
            }
        },
        "models.SearchRoleResponse": {
            "type": "object",
            "properties": {
                "roles": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/models.RoleModel"
                    }
                }
            }
        },
        "models.SearchScriptResponse": {
            "type": "object",
            "properties": {
                "scripts": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/models.Script"
                    }
                }
            }
        },
        "models.SearchWorkflowResponse": {
            "type": "object",
            "properties": {
                "workflows": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/models.WorkflowModel"
                    }
                }
            }
        },
        "models.SearchWorkflowScenarioResponse": {
            "type": "object",
            "properties": {
                "scenarios": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/models.WorkflowScenario"
                    }
                }
            }
        },
        "models.SortMapElement": {
            "type": "object",
            "properties": {
                "id": {
                    "type": "integer"
                },
                "weight": {
                    "type": "integer"
                }
            }
        },
        "models.SortMapLayer": {
            "type": "object",
            "properties": {
                "id": {
                    "type": "integer"
                },
                "weight": {
                    "type": "integer"
                }
            }
        },
        "models.UpdateDevice": {
            "type": "object",
            "properties": {
                "description": {
                    "type": "string"
                },
                "device": {
                    "type": "object",
                    "$ref": "#/definitions/models.ParentDevice"
                },
                "name": {
                    "type": "string"
                },
                "node": {
                    "type": "object",
                    "$ref": "#/definitions/models.NewDeviceNode"
                },
                "properties": {
                    "type": "object"
                },
                "status": {
                    "type": "string"
                },
                "type": {
                    "type": "string"
                }
            }
        },
        "models.UpdateDeviceAction": {
            "type": "object",
            "properties": {
                "description": {
                    "type": "string"
                },
                "device": {
                    "type": "object",
                    "$ref": "#/definitions/models.DeviceActionDevice"
                },
                "name": {
                    "type": "string"
                },
                "script": {
                    "type": "object",
                    "$ref": "#/definitions/models.DeviceActionScript"
                }
            }
        },
        "models.UpdateDeviceState": {
            "type": "object",
            "properties": {
                "description": {
                    "type": "string"
                },
                "device": {
                    "type": "object",
                    "$ref": "#/definitions/models.DeviceStateDevice"
                },
                "system_name": {
                    "type": "string"
                }
            }
        },
        "models.UpdateFlowModel": {
            "type": "object",
            "properties": {
                "description": {
                    "type": "string"
                },
                "id": {
                    "type": "integer"
                },
                "name": {
                    "type": "string"
                },
                "scenario": {
                    "type": "object",
                    "properties": {
                        "id": {
                            "type": "integer"
                        }
                    }
                },
                "status": {
                    "type": "string"
                },
                "workflow": {
                    "type": "object",
                    "properties": {
                        "id": {
                            "type": "integer"
                        }
                    }
                }
            }
        },
        "models.UpdateImage": {
            "type": "object",
            "properties": {
                "id": {
                    "type": "integer"
                },
                "image": {
                    "type": "string"
                },
                "mime_type": {
                    "type": "string"
                },
                "name": {
                    "type": "string"
                },
                "size": {
                    "type": "integer"
                },
                "thumb": {
                    "type": "string"
                },
                "title": {
                    "type": "string"
                },
                "url": {
                    "type": "string"
                }
            }
        },
        "models.UpdateMap": {
            "type": "object",
            "properties": {
                "description": {
                    "type": "string"
                },
                "id": {
                    "type": "integer"
                },
                "name": {
                    "type": "string"
                },
                "options": {
                    "type": "object",
                    "$ref": "#/definitions/models.MapOptions"
                }
            }
        },
        "models.UpdateMapElement": {
            "type": "object",
            "properties": {
                "description": {
                    "type": "string"
                },
                "graph_settings": {
                    "type": "object",
                    "$ref": "#/definitions/models.MapElementGraphSettings"
                },
                "id": {
                    "type": "integer"
                },
                "layer_id": {
                    "type": "integer"
                },
                "map": {
                    "type": "object",
                    "$ref": "#/definitions/models.Map"
                },
                "map_id": {
                    "type": "integer"
                },
                "name": {
                    "type": "string"
                },
                "prototype": {
                    "type": "object",
                    "$ref": "#/definitions/models.Prototype"
                },
                "prototype_id": {
                    "type": "integer"
                },
                "prototype_type": {
                    "type": "string"
                },
                "status": {
                    "type": "string"
                },
                "weight": {
                    "type": "integer"
                }
            }
        },
        "models.UpdateMapLayer": {
            "type": "object",
            "properties": {
                "description": {
                    "type": "string"
                },
                "id": {
                    "type": "integer"
                },
                "map": {
                    "type": "object",
                    "$ref": "#/definitions/models.Map"
                },
                "name": {
                    "type": "string"
                },
                "status": {
                    "type": "string"
                }
            }
        },
        "models.UpdateNodeModel": {
            "type": "object",
            "properties": {
                "description": {
                    "type": "string"
                },
                "id": {
                    "type": "integer"
                },
                "ip": {
                    "type": "string"
                },
                "name": {
                    "type": "string"
                },
                "port": {
                    "type": "integer"
                },
                "status": {
                    "type": "string"
                }
            }
        },
        "models.UpdateRole": {
            "type": "object",
            "properties": {
                "description": {
                    "type": "string"
                },
                "name": {
                    "type": "string"
                },
                "parent": {
                    "type": "object",
                    "properties": {
                        "name": {
                            "type": "string"
                        }
                    }
                }
            }
        },
        "models.UpdateScript": {
            "type": "object",
            "properties": {
                "description": {
                    "type": "string"
                },
                "lang": {
                    "type": "string"
                },
                "name": {
                    "type": "string"
                },
                "source": {
                    "type": "string"
                }
            }
        },
        "models.UpdateUser": {
            "type": "object",
            "properties": {
                "avatar": {
                    "type": "object",
                    "$ref": "#/definitions/models.NewUserImage"
                },
                "email": {
                    "type": "string"
                },
                "first_name": {
                    "type": "string"
                },
                "id": {
                    "type": "integer"
                },
                "lang": {
                    "type": "string"
                },
                "last_name": {
                    "type": "string"
                },
                "meta": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/models.NewUserMeta"
                    }
                },
                "nickname": {
                    "type": "string"
                },
                "password": {
                    "type": "string"
                },
                "password_repeat": {
                    "type": "string"
                },
                "role": {
                    "type": "object",
                    "$ref": "#/definitions/models.NewUserRole"
                },
                "status": {
                    "type": "string"
                }
            }
        },
        "models.UpdateWorkflow": {
            "type": "object",
            "properties": {
                "description": {
                    "type": "string"
                },
                "name": {
                    "type": "string"
                },
                "status": {
                    "type": "string"
                }
            }
        },
        "models.UserByIdModelMeta": {
            "type": "object",
            "properties": {
                "key": {
                    "type": "string"
                },
                "value": {
                    "type": "string"
                }
            }
        },
        "models.UserByIdModelParent": {
            "type": "object",
            "properties": {
                "id": {
                    "type": "integer"
                },
                "nickname": {
                    "type": "string"
                }
            }
        },
        "models.UserFullModel": {
            "type": "object",
            "properties": {
                "authentication_token": {
                    "type": "string"
                },
                "created_at": {
                    "type": "string"
                },
                "current_sign_in_at": {
                    "type": "string"
                },
                "current_sign_in_ip": {
                    "type": "string"
                },
                "deleted_at": {
                    "type": "string"
                },
                "email": {
                    "type": "string"
                },
                "first_name": {
                    "type": "string"
                },
                "history": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/models.UserHistory"
                    }
                },
                "id": {
                    "type": "integer"
                },
                "image": {
                    "type": "object",
                    "$ref": "#/definitions/models.Image"
                },
                "lang": {
                    "type": "string"
                },
                "last_name": {
                    "type": "string"
                },
                "last_sign_in_at": {
                    "type": "string"
                },
                "last_sign_in_ip": {
                    "type": "string"
                },
                "meta": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/models.UserByIdModelMeta"
                    }
                },
                "nickname": {
                    "type": "string"
                },
                "reset_password_sent_at": {
                    "type": "string"
                },
                "role": {
                    "type": "object",
                    "$ref": "#/definitions/models.RoleModel"
                },
                "role_name": {
                    "type": "string"
                },
                "sign_in_count": {
                    "type": "integer"
                },
                "status": {
                    "type": "string"
                },
                "updated_at": {
                    "type": "string"
                },
                "user": {
                    "type": "object",
                    "$ref": "#/definitions/models.UserByIdModelParent"
                }
            }
        },
        "models.UserHistory": {
            "type": "object",
            "properties": {
                "ip": {
                    "type": "string"
                },
                "time": {
                    "type": "string"
                }
            }
        },
        "models.UserListModel": {
            "type": "object",
            "properties": {
                "items": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/models.UserShotModel"
                    }
                },
                "meta": {
                    "type": "object",
                    "properties": {
                        "limit": {
                            "type": "integer"
                        },
                        "objects_count": {
                            "type": "integer"
                        },
                        "offset": {
                            "type": "integer"
                        }
                    }
                }
            }
        },
        "models.UserShotModel": {
            "type": "object",
            "properties": {
                "created_at": {
                    "type": "string"
                },
                "email": {
                    "type": "string"
                },
                "first_name": {
                    "type": "string"
                },
                "id": {
                    "type": "integer"
                },
                "image": {
                    "type": "object",
                    "$ref": "#/definitions/models.Image"
                },
                "lang": {
                    "type": "string"
                },
                "last_name": {
                    "type": "string"
                },
                "nickname": {
                    "type": "string"
                },
                "role": {
                    "type": "object",
                    "$ref": "#/definitions/models.RoleModel"
                },
                "role_name": {
                    "type": "string"
                },
                "status": {
                    "type": "string"
                },
                "updated_at": {
                    "type": "string"
                },
                "user": {
                    "type": "object",
                    "$ref": "#/definitions/models.UserByIdModelParent"
                }
            }
        },
        "models.UserUpdateStatusRequest": {
            "type": "object",
            "properties": {
                "status": {
                    "type": "string"
                }
            }
        },
        "models.WorkerModel": {
            "type": "object",
            "properties": {
                "created_at": {
                    "type": "string"
                },
                "device_action": {
                    "type": "object",
                    "$ref": "#/definitions/models.DeviceAction"
                },
                "device_action_id": {
                    "type": "integer"
                },
                "flow": {
                    "type": "object",
                    "$ref": "#/definitions/models.FlowModel"
                },
                "flow_id": {
                    "type": "integer"
                },
                "id": {
                    "type": "integer"
                },
                "name": {
                    "type": "string"
                },
                "status": {
                    "type": "string"
                },
                "time": {
                    "type": "string"
                },
                "updated_at": {
                    "type": "string"
                },
                "workflow": {
                    "type": "object",
                    "$ref": "#/definitions/models.WorkflowModel"
                },
                "workflow_id": {
                    "type": "integer"
                }
            }
        },
        "models.WorkflowModel": {
            "type": "object",
            "properties": {
                "created_at": {
                    "type": "string"
                },
                "description": {
                    "type": "string"
                },
                "id": {
                    "type": "integer"
                },
                "name": {
                    "type": "string"
                },
                "scenario": {
                    "type": "object",
                    "$ref": "#/definitions/models.WorkflowScenario"
                },
                "scenarios": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/models.WorkflowScenario"
                    }
                },
                "scripts": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/models.Script"
                    }
                },
                "status": {
                    "type": "string"
                },
                "updated_at": {
                    "type": "string"
                }
            }
        },
        "models.WorkflowScenario": {
            "type": "object",
            "properties": {
                "created_at": {
                    "type": "string"
                },
                "id": {
                    "type": "integer"
                },
                "name": {
                    "type": "string"
                },
                "scripts": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/models.Script"
                    }
                },
                "system_name": {
                    "type": "string"
                },
                "updated_at": {
                    "type": "string"
                },
                "workflow_id": {
                    "type": "integer"
                }
            }
        },
        "models.WorkflowScenarioListModel": {
            "type": "object",
            "properties": {
                "scenarios": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/models.WorkflowScenario"
                    }
                }
            }
        },
        "models.WorkflowUpdateWorkflowScenario": {
            "type": "object",
            "properties": {
                "workflow_scenario_id": {
                    "type": "integer"
                }
            }
        }
    },
    "securityDefinitions": {
        "ApiKeyAuth": {
            "type": "apiKey",
            "name": "Authorization",
            "in": "header"
        },
        "BasicAuth": {
            "type": "basic"
        }
    }
}`

type swaggerInfo struct {
	Version     string
	Host        string
	BasePath    string
	Title       string
	Description string
}

// SwaggerInfo holds exported Swagger Info so clients can modify it
var SwaggerInfo swaggerInfo

type s struct{}

func (s *s) ReadDoc() string {
	t, err := template.New("swagger_info").Parse(doc)
	if err != nil {
		return doc
	}

	var tpl bytes.Buffer
	if err := t.Execute(&tpl, SwaggerInfo); err != nil {
		return doc
	}

	return tpl.String()
}

func init() {
	swag.Register(swag.Name, &s{})
}
