...
func (u User) ToUserFull(user *m.User) (result *stub.ApiUserFull) {
	...
	result = &stub.ApiUserFull{
		...
		FirstName:           pkgCommon.String(user.FirstName),
		LastName:            pkgCommon.String(user.LastName),
		Email:               user.Email,
		Status:              user.Status,
		...
		Lang:                user.Lang,
		AuthenticationToken: pkgCommon.StringValue(user.AuthenticationToken),
	...
	if user.LastSignInIp != "" {
		result.LastSignInIp = pkgCommon.String(user.LastSignInIp)
	}

	if user.CurrentSignInIp != "" {
		result.CurrentSignInIp = pkgCommon.String(user.CurrentSignInIp)
	}
