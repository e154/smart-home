...
func New(t time.Time) (moonP *Moon) {
	...
	var m = fixangle(n + elonge - elongp)    // Convert from perigee co-orginates to epoch 1980.0
	...
	var f = (1 + eccent*cos(deg2rad(ec))) / (1 - eccent*eccent) // Orbital distance factor
	...
	var ev = 1.2739 * sin(deg2rad(2*(ml-lambdasun)-mm)) // Evection
	var ae = 0.1858 * sin(deg2rad(m))                   // Annual equation
	var a3 = 0.37 * sin(deg2rad(m))                     // Correction term
	var mmP = mm + ev - ae - a3                         // Corrected anomaly
	var mec = 6.2886 * sin(deg2rad(mmP))                // Correction for the equation of the centre
	var a4 = 0.214 * sin(deg2rad(2*mmP))                // Another correction term
	var lP = ml + ev + mec - ae + a4                    // Corrected longitude
	var v = 0.6583 * sin(deg2rad(2*(lP-lambdasun)))     // Variation
	var lPP = lP + v                                    // True longitude

	// Calculation of the phase of the Moon
	var moonAge = lPP - lambdasun                   // Age of the Moon in degrees
	var moonPhase = (1 - cos(deg2rad(moonAge))) / 2 // Phase of the Moon

	// Distance of moon from the centre of the Earth
	var moonDist = (msmax * (1 - mecc*mecc)) / (1 + mecc*cos(deg2rad(mmP+mec)))
