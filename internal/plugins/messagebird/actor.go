...
func (e *Actor) Send(phone string, message *m.Message) (err error) {
	...
	if common.TestMode() {
		...
		if msg, err = sms.Create(client, e.Name, []string{phone}, attr[AttrBody].String(), params); err != nil {
			...
			if !ok {
				err = fmt.Errorf("can`t static cast to messagebird.ErrorResponse: %w", err)
				return
			}

			err = fmt.Errorf("%s: %w", mbErr.Errors[0].Description, err)
			return
		}
	}
...
func (e *Actor) GetStatus(smsId string) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("failed read sms: %w", err)
	}
