...
func (p *WeatherOwm) FetchData(name string, lat, lon float64, now time.Time, settings map[string]*m.Attribute) (zone Zone, err error) {
	...
	if lat == 0 || lon == 0 {
		err = fmt.Errorf("zero positions: %w", apperr.ErrBadRequestParams)
		return
	}
	...
	if body, err = p.fetchFromServer(lat, lon, settings); err != nil {
		err = fmt.Errorf("%s: fetch from server failed", err.Error())
		return
	}
	...
	if err = json.Unmarshal(body, zone.Weatherdata); err != nil {
		err = fmt.Errorf("%s: failed unmarshal response body", err.Error())
		return
	}

	// save to storage
	if err = p.saveToLocalStorage(zone); err != nil {
		err = fmt.Errorf("%s: save to local storage failed", err.Error())
	}
