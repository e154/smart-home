...
import (
	...
	app2 "github.com/e154/smart-home/internal/common/app"
...
func (b *Backup) RollbackChanges() (err error) {
	...
	if err = db.Exec(`DROP EXTENSION IF EXISTS timescaledb CASCADE;`).Error; err != nil {
		err = fmt.Errorf("%s: failed drop extension timescaledb", err.Error())
		return
	}

	if err = db.Exec(`ALTER SCHEMA "public_old" RENAME TO "public";`).Error; err != nil {
		err = fmt.Errorf("%s: failed rename public scheme", err.Error())
		return
	}

	if err = db.Exec(`DROP SCHEMA IF EXISTS "public_old" CASCADE;`).Error; err != nil {
		err = fmt.Errorf("%s: failed drop schema", err.Error())
		return
	}

	if err = db.Exec(`CREATE EXTENSION IF NOT EXISTS timescaledb CASCADE;`).Error; err != nil {
		err = fmt.Errorf("%s: failed create extension if not exists timescaledb", err.Error())
	}
...
func (b *Backup) RestoreFile(name string) (err error) {
	...
	if os.IsNotExist(err) {
		err = fmt.Errorf("path %s: %w", file, apperr.ErrBackupNotFound)
		return
	}

	tmpDir := path.Join(os.TempDir(), "smart_home")
	if err = commonPkg.Unzip(file, tmpDir); err != nil {
		err = fmt.Errorf("%s: failed unzip file %s", err.Error(), file)
		return
	}
	...
	if err = db.Exec(`CREATE SCHEMA IF NOT EXISTS "public";`).Error; err != nil {
		err = fmt.Errorf("%s: failed create public schema", err.Error())
		return
	}

	if err = db.Exec(`DROP SCHEMA IF EXISTS "public_old" CASCADE;`).Error; err != nil {
		err = fmt.Errorf("%s: failed drop public_old schema", err.Error())
		return
	}
...
func (b *Backup) RestoreFromChunks() {
	...
	ok, err := commonPkg.CheckZip(fileName)
	if err != nil {
		log.Error(err.Error())
		return
	}
...
func (b *Backup) SendFileToTelegram(params events.CommandSendFileToTelegram) {
	...
	b.eventBus.Publish("system/plugins/notify", notifyCommon.Message{
		EntityId: commonPkg.NewEntityId(params.EntityId.String()),
		Attributes: map[string]interface{}{
			"file_path": fileList,
		},
	})
