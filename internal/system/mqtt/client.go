...
type client struct {
	...
	subscribers     map[string]mqttType.MessageHandler
}
...
func NewClient(mqtt *Mqtt, name string) mqttType.MqttCli {
	return &client{
		...
		name:            name,
		subscribersLock: &sync.Mutex{},
		subscribers:     make(map[string]mqttType.MessageHandler),
	}
}
...
// Subscribe ...
func (m *client) Subscribe(topic string, handler mqttType.MessageHandler) (err error) {
