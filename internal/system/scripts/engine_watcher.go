...
var _ scripts.EngineWatcher = (*EngineWatcher)(nil)
...
func (w *EngineWatcher) Stop() {
	if w.engine.Script() != nil && w.engine.Script().Id != 0 {
		_ = w.eventBus.Unsubscribe(fmt.Sprintf("system/models/scripts/%d", w.engine.Script().Id), w.eventHandler)
	}
}
