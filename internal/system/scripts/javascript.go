...
import (
	...
	"github.com/e154/smart-home/internal/system/scripts/eventloop"
...
func (j *Javascript) Do() (result string, err error) {
	...
	if err != nil {
		if j.engine.ScriptId() != 0 {
			err = fmt.Errorf("script id:%d :%w", j.engine.ScriptId(), err)
		}
	}
	return
}
...
func (j *Javascript) RunProgram(name string) (result string, err error) {
	...
	if !ok {
		err = fmt.Errorf("name \"%s\": %w", name, apperr.ErrNotFound)
		return
	}
...
func (j *Javascript) unsafeRun(program *goja.Program) (result string, err error) {
	...
	if err != nil {
		err = fmt.Errorf("unsafeRun: %w", err)
		return
	}
