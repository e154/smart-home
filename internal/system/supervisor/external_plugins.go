...
func (p *ExternalPlugins) uploadPlugin(ctx context.Context, reader *bufio.Reader) (plugin *m.Plugin, err error) {
	...
	if err = p.checkArchive(common.CopyBuffer(buffer), manifest); err != nil {
		return nil, errors.Wrap(fmt.Errorf("archive failed manifest check"), err.Error())
	}
	...
	if err = os.MkdirAll(pluginDir, 0755); err != nil {
		return nil, errors.Wrap(fmt.Errorf("create dir %s failed", pluginDir), err.Error())
	}
	...
	if err = p.extractFileFromArchive(common.CopyBuffer(buffer), manifest.Plugin, to); err != nil {
		return nil, errors.Wrap(fmt.Errorf("copy plugin file %s to %s failed", manifest.Plugin, to), err.Error())
	}
	...
	for _, item := range manifest.Libs {
		...
		if err = p.extractFileFromArchive(common.CopyBuffer(buffer), item, to); err != nil {
			return nil, errors.Wrap(fmt.Errorf("copy library file %s to %s failed", item, to), err.Error())
		}
	}
	...
	for _, item := range manifest.Assets {
		...
		if err = p.extractFileFromArchive(common.CopyBuffer(buffer), item, to); err != nil {
			return nil, errors.Wrap(fmt.Errorf("copy asset file %s to %s failed", item, to), err.Error())
		}
	}
	...
	if err = p.extractFileFromArchive(common.CopyBuffer(buffer), "manifest.json", to); err != nil {
		return nil, errors.Wrap(fmt.Errorf("copy manifest file %s to %s failed", "manifest.json", to), err.Error())
	}
...
func (p *ExternalPlugins) removeExternalPlugin(ctx context.Context, pluginName string) (err error) {
	...
	if !plugin.External {
		err = errors.Wrap(apperr.ErrPluginDelete, "you can only remove external plugins")
		return
	}
	...
	if err != nil {
		return errors.Wrap(fmt.Errorf("can't open fole %s", filepath.Join(pluginDir, "manifest.json")), err.Error())
	}
	defer file.Close()
...
func (p *ExternalPlugins) readManifest(r io.Reader) (*plugins.PluginManifest, error) {
	...
	if err != nil {
		return nil, errors.Wrap(fmt.Errorf("read manifest failed"), err.Error())
	}

	if err = json.Unmarshal(data, manifest); err != nil {
		return nil, errors.Wrap(fmt.Errorf("unmarshal manifest failed"), err.Error())
	}
...
func (p *ExternalPlugins) readArchiveManifest(buffer io.Reader) (*plugins.PluginManifest, error) {
	...
	for {
		...
		if err != nil {
			return nil, errors.Wrap(fmt.Errorf("extract tar failed"), err.Error())
		}

		if header == nil {
			return nil, errors.Wrap(fmt.Errorf("extract tar failed"), "header is nil")
		}
...
func (p *ExternalPlugins) extractFileFromArchive(buffer io.Reader, fileName, destName string) error {
	...
	for {
		...
		if header == nil {
			return errors.Wrap(fmt.Errorf("extract tar failed"), "header is nil")
		}
		...
		case tar.TypeReg:
			...
			if err != nil {
				return errors.Wrap(fmt.Errorf("can't create file %s", destName), err.Error())
			}
			defer dest.Close()
