...
export default defineComponent({
  ...
  props: {
    ...
    showUpPagination: propTypes.number.def(0),
  ...
  setup(props, { attrs, slots, emit, expose }) {
    ...
    return () => (
      <div v-loading={unref(getProps).loading}>
        {unref(getProps).pagination && unref(getProps).showUpPagination &&
            unref(getProps).data?.length > unref(getProps).showUpPagination  ? (
            <ElPagination
