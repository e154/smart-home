...
export default {
    ...
    modelValue: {
      handler(newItems) {
        ...
        if (!this.internallySet) {
          this._sync(this.items, this.copiedItems);
        }
      },
      deep: true,
      immediate: true,
  ...
  methods: {
    ...
    _setup() {
      ...
      this.$nextTick(() => {
        GridStore.setItemsForGridId(this.gridKey, this.items);
      });
    },
    /**
     * @private
    ...
    _setupNonReactiveProps() {
       ...
       * @type {string}
       */
      this.gridKey = new UUID().getDashFreeUUID().replace(/-/g, '');
    },
    /**
    ...
    _reOrderWithItem(item) {
      ...
      return this.items.reduce((accum, a) => {
