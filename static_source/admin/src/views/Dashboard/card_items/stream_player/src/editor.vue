...
const onChangeValue = (val) => {
  currentItem.value.payload.streamPlayer.attribute = val;
}
...
<template>
  <div>
    ...
    <ElRow>
      <ElCol>
        <ElFormItem :label="$t('dashboard.editor.type')" prop="type">
          <ElSelect
              v-model="currentItem.payload.streamPlayer.playerType"
    ...
    <ElRow v-if="currentItem.payload.streamPlayer.playerType === playerType.youtube">
      <ElCol>
        <ElFormItem :label="$t('dashboard.editor.attrField')" prop="value">
          <KeysSearch v-model="currentItem.payload.streamPlayer.attribute" :obj="currentItem.lastEvent"
