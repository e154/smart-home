...
func NewNode(model *m.Node) *Node {
	node := &Node{
		Node: model,
		ch:   make(chan string),
	}
...
func (n *Node) Smartbus(device *m.Device, return_result bool, command []byte) (result SmartbusResult) {
...
func (n *Node) Modbus(device *m.Device, return_result bool, command []byte) (result SmartbusResult) {
